...
router.get('/', (req, res) => {
  knex
    .select(
      'shake_comments.id as post_id',
      'shake_comments.title',
      'shake_comments.content',
      'shake_comments.updated_at',
      'users.id as user_id',
      'users.avatar_url',
      'users.username'
    )
    .from('shake_comments')
    .leftJoin('users', 'shake_comments.user_id', 'users.id')
    .orderBy('shake_comments.id', 'desc')
    .then(posts => {
