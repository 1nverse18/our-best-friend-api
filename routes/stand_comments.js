...
router.get('/', (req, res) => {
  knex
    .select(
      'stand_comments.id as post_id',
      'stand_comments.title',
      'stand_comments.content',
      'stand_comments.updated_at',
      'users.id as user_id',
      'users.avatar_url',
      'users.username'
    )
    .from('stand_comments')
    .leftJoin('users', 'stand_comments.user_id', 'users.id')
    .orderBy('stand_comments.id', 'desc')
    .then(posts => {
